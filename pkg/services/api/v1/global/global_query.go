...
func GenerateQueryParking(queryParams QueryParams) string {
...
func GenerateQueryParkingData(queryParams QueryParams) string {
	...
	sql := fmt.Sprintf(`SELECT "invoiceId", DATE_PART('day', '%s'::timestamp - "enteredDate"::timestamp) * 24 + DATE_PART('hour', '%s'::timestamp - "enteredDate"::timestamp) as "timeDiff", "enteredDate" from "dataParking"`, queryParams["dateTime"], queryParams["dateTime"])
